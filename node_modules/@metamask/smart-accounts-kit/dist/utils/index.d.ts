export { d as decodeCaveat, a as deploySmartAccountsEnvironment, o as overrideDeployedEnvironment } from '../caveats-B2OssjV4.js';
export { D as DELEGATION_ABI_TYPE, d as DELEGATION_ABI_TYPE_COMPONENTS, e as DELEGATION_ARRAY_ABI_TYPE, f as DecodedRevertReason, g as DelegationStruct, S as SIGNABLE_DELEGATION_TYPED_DATA, h as decodeDelegation, i as decodeDelegations, j as decodeRevertData, k as decodeRevertReason, l as encodeDelegation, m as encodeDelegations, n as hashDelegation, t as toDelegation, o as toDelegationStruct } from '../decodeRevertReason-dOTpJx2h.js';
import { p as Call, I as Implementation, S as SmartAccountsEnvironment, q as DeployParams } from '../resolveCaveats-A9xC29l8.js';
export { r as CaveatBuilder, s as CaveatBuilderConfig, t as CoreCaveatBuilder, e as ExecutionStruct, u as createCaveatBuilder, v as encodeBatchExecution, w as encodeExecutionCalldata, x as encodeExecutionCalldatas, y as encodeSingleExecution } from '../resolveCaveats-A9xC29l8.js';
export { S as SIGNABLE_USER_OP_TYPED_DATA } from '../userOp-DKgjzz7B.js';
import { Address, Hex } from 'viem';
export { DELEGATION_TYPEHASH } from '@metamask/delegation-core';
import '../index-LqiEgBjV.js';
import '@metamask/delegation-abis';
import 'viem/account-abstraction';
import 'viem/chains';

declare const SIGNATURE_ABI_PARAMS: readonly [{
    readonly type: "bytes32";
}, {
    readonly type: "uint256";
}, {
    readonly type: "uint256";
}, {
    readonly type: "bytes";
}, {
    readonly type: "bool";
}, {
    readonly type: "string";
}, {
    readonly type: "string";
}, {
    readonly type: "uint256";
}];
type AuthenticatorFlags = {
    userVerified: boolean;
};

declare const encodeCalls: (calls: readonly Call[]) => `0x${string}`;
declare const encodeCallsForCaller: (caller: Address, calls: readonly Call[]) => Promise<Hex>;

declare const getCounterfactualAccountData: <TImplementation extends Implementation>({ factory, implementations, implementation, deployParams, deploySalt, }: {
    factory: Address;
    implementations: SmartAccountsEnvironment["implementations"];
    implementation: TImplementation;
    deployParams: DeployParams<TImplementation>;
    deploySalt: Hex;
}) => Promise<{
    factoryData: Hex;
    address: Address;
}>;

declare function generateSalt(): `0x${string}`;

export { type AuthenticatorFlags, SIGNATURE_ABI_PARAMS, encodeCalls, encodeCallsForCaller, generateSalt, getCounterfactualAccountData };
